...
playstore.drop_duplicates(subset = 'App',inplace=True,keep='first') 
...
def index():
    ...
    stats = {
        ...
        'rev_table' : playstore.nlargest(10,'Reviews').reset_index()[['Category','App','Reviews','Rating']].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    plt.xlabel('Installs')
    plt.ylabel('Rating')
    plt.title('Hubungan antara Jumlah Instalasi dengan Rating Aplikasi')
    plt.savefig('install_rating.png',bbox_inches="tight")
